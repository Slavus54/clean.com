...
import {ESTATE_TYPES, LOCALITIES, SEARCH_PERCENT, PROJECT_TITLE, VIEW_CONFIG, token} from '../../../env/env'
...
import {getRentalsQ} from '../../../graphql/pages/RentalPageQueries'
...
const Rentals: React.FC = () => {
    ...
    const [category, setCategory] = useState<string>(ESTATE_TYPES[0])
    const [format, setFormat] = useState<string>(LOCALITIES[0])
    const [region, setRegion] = useState<string>(towns[0].title)
    const [cords, setCords] = useState<Cords>(towns[0].cords)

    const centum = new Centum()

    const {data, loading} = useQuery(getRentalsQ)
    ...
    useEffect(() => {
        ...
        centum.title('Rentals', PROJECT_TITLE)
    }, [data])
    ...
    useMemo(() => {
        if (rentals !== null) {
            let result: any[] = rentals.filter(el => el.region === region)
    ...
    }, [rentals, title, category, format, region])
    ...
    return (
        <>
            ...
            <div className='items small'>
                {ESTATE_TYPES.map(el => <div onClick={() => setCategory(el)} className={el === category ? 'item label active' : 'item label'}>{el}</div>)}
            </div>
            ...
            <DataPagination initialItems={filtered} setItems={setFiltered} label='Rentals on map:' />
